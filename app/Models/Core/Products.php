...
class Products extends Model
{
  ...
  public function inventorydetailsApi($barcode){
    $setting = new Setting();
    $myVarsetting = new SiteSettingController($setting);
    $myVaralter = new AlertController($setting);
    $language_id      =   '1';
    ...
    $product = DB::table('products')
            ...
            if ($product_id[0]->products_id != null) {

                $product->where('products.products_id', '=', $product_id[0]->products_id);
   ...
   // if($result['products'][0]->products_type!=1){
    ...
    $result['min_level'] = $min_level;
    $result['max_level'] = $max_level;
    $products_attribute = DB::table('products_attributes')->where('products_id', '=', $product_id[0]->products_id)->get();
    $products_attribute = $products_attribute->unique('options_id')->keyBy('options_id');
    if(count($products_attribute)>0){
        $index2 = 0;
        foreach($products_attribute as $attribute_data){
          $option_name = DB::table('products_options')
            ...
            if(count($option_name)>0){
                ...
                // fetch all attributes add join from products_options_values table for option value name
                $attributes_value_query = DB::table('products_attributes')
                ->where('products_id', '=', $product_id[0]->products_id)
                ->where('options_id', '=', $attribute_data->options_id)
                ->get();
                foreach($attributes_value_query as $products_option_value){
                    $option_value = DB::table('products_options_values')
                    ...
                    if(count($option_value)>0){
                        $attributes = DB::table('products_attributes')
                        ->where([['products_id', '=', $product_id[0]->products_id], ['options_id', '=', $attribute_data->options_id], ['options_values_id', '=', $products_option_value->options_values_id]])
                        ->get();
                        $temp_i['products_attributes_id'] = $attributes[0]->products_attributes_id;
                        $temp_i['id'] = $products_option_value->options_values_id;
                        $temp_i['value'] = $option_value[0]->products_options_values_name;
  ...
  public function addnewstockApi($request){
    $barcode= $request->barcode;
    $product_id= DB::table('products')
    ->where('barcode',$barcode)
    ->select('products_id')
    ->get();
    $products_id = $product_id[0]->products_id;
    $language_id     =   1;
    $product = DB::table('products')
                ->leftJoin('products_description', 'products_description.products_id', '=', 'products.products_id')
                ->leftJoin('manufacturers', 'manufacturers.manufacturers_id', '=', 'products.manufacturers_id')
  ...
  public function displayProductImagesApi($request){
    $barcode = $request->barcode;
    $result['data'] = array('barcode'=>$barcode);
  ...
  public function addProductImages($products_id){
    ...
    $products_attributes = DB::table('products_attributes')
        ...
        ->where('products_options_descriptions.options_name', '=', 'Color')
  ...
  public function addProductImagesApi($request){
    $barcode= $request->barcode;
    $result['data'] = array('barcode'=>$barcode);
    $products_images = DB::table('products_images')
        ->where('barcode','=', $barcode)
  ...
  public function insertProductImages($request){
    $product_id = $request->products_id;
         ...
         if($request->products_options_id !== null){
         ...
         DB::table('products_images')->insert([
             ...
             'products_attributes_id' => $products_options_id
         ]);

       return $product_id;
  }
  public function insertProductImagesApi($request){
    $barcode = $request->barcode;
         if($request->image_id !== null){
             $image = $request->image_id;
         }else{
         ...
         $product_id= DB::table('products')
                      ...
                      ->select('products_id')
                      ->get();
         //$product_id= DB::select("SELECT products_id FROM  products WHERE barcode='$barcode'");
         DB::table('products_images')->insert([
             'products_id' => $product_id[0]->products_id,
             'image' => $image,
             'htmlcontent' => $request->htmlcontent,
  ...
  public function addnewdefaultattributeApi($request){
    $language_id = 1;
    $products_attributes = '';
    $barcode= $request->barcode;
    $product_id= DB::table('products')
    ...
    ->select('products_id')
    ->get();
    if(!empty($request->products_options_id) and !empty($request->barcode) and !empty($request->products_options_values_id)){
      $checkRecord = DB::table('products_attributes')->where([
          'options_id' => $request->products_options_id,
          'products_id' => $product_id[0]->products_id,
          'options_values_id' => $request->products_options_values_id,
        ...
        if(count($checkRecord)>0){
          ...
          $products_attributes = DB::table('products_attributes')
              ...
              ->where('products_options_descriptions.language_id', '=', $language_id)
              ->where('products_options_values_descriptions.language_id', '=', $language_id)
              ->where('products_attributes.barcode', '=', $request->barcode)
              ->where('products_attributes.is_default', '=', '1')
              ->orderBy('products_attributes_id', 'DESC')
              ->get();
        }
  ...
  public function showoptionsApi($request){
    $setting = new Setting();
    $myVarsetting = new SiteSettingController($setting);
    $myVaralter = new AlertController($setting);
    $barcode= $request->barcode;
    ...
    ->select('products_id')
    ->get();
        if(!empty($request->products_options_id) and !empty($request->barcode) and !empty($request->products_options_values_id) and isset($request->options_values_price)){
              $checkRecord = DB::table('products_attributes')->where([
                  'options_id' => $request->products_options_id,
                  'options_values_id' => $request->products_options_values_id,
                  'products_id' => $product_id[0]->products_id
