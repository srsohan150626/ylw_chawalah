...
class MediaController extends Controller
{
    ...
    public function display()
    {
        ...
        //dd($settings);
        return view("admin.media.index")->with('settings', $settings);
    }
        ...
        if ($request->hasFile('file') and in_array($request->file->extension(), $extensions)) {
            ...
            switch (true) {
                case ($width >= $AllImagesSettingData[5]->value || $height >= $AllImagesSettingData[4]->value):
                    $tuhmbnail = $this->storeThumbnial($image, $filename, $directory, $filename);
                    $mediumimage = $this->storeMedium($image, $filename, $directory, $filename);
                    $largeimage = $this->storeLarge($image, $filename, $directory, $filename);
                    break;
                case ($width >= $AllImagesSettingData[3]->value || $height >= $AllImagesSettingData[2]->value):
                    $tuhmbnail = $this->storeThumbnial($image, $filename, $directory, $filename);
                    $mediumimage = $this->storeMedium($image, $filename, $directory, $filename);
                    //                $storeLargeImage = $Images->Largerecord($filename,$image,$width,$height);
                    break;
...
//        $upload_success = $thumbnailImage->save($directory, $filename, 'public');
    ...
    public function detailimage($id)
    {
        ...
        return view('admin.media.detail')->with('result', $result);
